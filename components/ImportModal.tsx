...
const ImportModal: React.FC<ImportModalProps> = ({ onClose, onImport, pricingRules, accountGroups }) => {
  ...
  const validateData = (headers: string[], rows: string[][]): { data: EVTransaction[], errors: string[] } => {
    const errors: string[] = [];
    ...
    if (missingHeaders.length > 0) {
      return { data: [], errors: [`Missing: ${missingHeaders.join(', ')}`] };
    }
    ...
    return { data: validTransactions, errors: errors.slice(0, 5) };
  };
  ...
  const handleFile = (file: File) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const { data, errors } = validateData(lines[0].split(','), lines.slice(1).map(l => l.split(',')));
        ...
        <div className="p-6">
          {!parsedData.length ? (
            <div onDragEnter={onDrag} onDragLeave={onDrag} onDragOver={onDrag} onDrop={onDrop}
              ...
              <p className="font-black text-slate-800">{isProcessing ? 'Validating CSV...' : 'Drop your CSV export here'}</p>
            ...
            <div className="space-y-4">
              <div className="bg-emerald-50 p-5 rounded-2xl border border-emerald-100 flex items-center gap-4 animate-in slide-in-from-bottom-2">
                ...
                <div>
                   <p className="text-emerald-900 font-black leading-tight">{parsedData.length} Transactions Parsed</p>
